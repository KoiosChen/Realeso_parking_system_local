from .models import ParkingLot, Camera
from . import logger
...
def get_machine_room_by_area(permit_machine_room):
    ...
    return [(str(k.id), k.name)
            for k in ParkingLot.query.filter(ParkingLot.status != 0).all()
            if int(k.permit_value, 16) & int(permit_machine_room, 16) == int(k.permit_value, 16)]


def get_device_name():
    return [(str(k.id), k.device_name) for k in Camera.query.all()]
...
def get_device_info(machine_room_id):
    ...
    :return:
    """
    device_info = Camera.query.filter_by(machine_room_id=machine_room_id).all()
