...
def logically_free_parking_space(parking_lot_id):
    ...
    acctually_temporary_parking_space = parking_lot_info.parking_space_totally - fixed
    ...
    temporary_for_reserved_plates = current_remaining_parking_space + len(list(reserved_plates_already_in)) - len(
        reserved_plates)

    # free = (parking_lot_info.parking_space_totally - parking_lot_info.parking_space_fixed) * 0.6 - 1

    logger.info("Acctually temporary parking space: {} "
                "Car parking num: {} "
                "Reserved number plates: {} "
                "current_remaining_parking_space: {} "
                "temporary_for_reserved_plates: {}".format(acctually_temporary_parking_space, len(car_parking_plates),
                                                           reserved_plates, current_remaining_parking_space,
                                                           temporary_for_reserved_plates))
